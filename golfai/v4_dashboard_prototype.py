...
def build_mock_gauge():
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=78,
        number={"font": {"size": 34, "color": "#f5f7fa"}},
        gauge={
...
def build_mock_distance():
    ...
    fig.add_trace(go.Scatter(
        x=[102.5], y=[0.5],
        mode="markers+text",
        marker=dict(size=14, color="rgba(255,90,90,1)"),
        text=["Avg 102.5m"],
        textposition="top center",
        textfont=dict(color="white", size=12),
        showlegend=False
    ))
    fig.add_annotation(x=72, y=0.12, text="72m", showarrow=False, font=dict(color="white", size=11))
    fig.add_annotation(x=132, y=0.12, text="132m", showarrow=False, font=dict(color="white", size=11))
    fig.add_annotation(x=104, y=0.84, text="Reliable 95–113m", showarrow=False, font=dict(color="#dff7ea", size=12))
    fig.update_layout(
        height=165,
        margin=dict(l=8, r=8, t=8, b=8),
        paper_bgcolor="#142c34",
...
def build_mock_dispersion():
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(size=9, color="rgba(230,255,200,0.9)", line=dict(width=1, color="rgba(0,0,0,0.25)")),
        showlegend=False
    ))

    fig.add_trace(go.Scatter(x=[0.4], y=[110.5], mode="markers",
        marker=dict(size=24, color="rgba(255,70,70,0.18)"), showlegend=False))
    fig.add_trace(go.Scatter(x=[0.4], y=[110.5], mode="markers",
        marker=dict(size=14, color="rgba(255,70,70,1)", symbol="diamond",
    ...
    for col, (label, value, delta, color) in zip(cols, cards):
        with col:
            ...
            <div style="
                ...
                min-height: 92px;
            ">
                <div style="color:#c7d4da; font-size:12px; margin-bottom:8px;">{label}</div>
                <div style="color:#f4f7fa; font-size:26px; font-weight:800; line-height:1;">{value}</div>
                <div style="margin-top:8px; color:{color}; font-weight:700; font-size:13px;">{delta}</div>
            </div>
...
def render_v4_dashboard_prototype():
    ...
    ">
        <div>
            <div style="font-size:20px;font-weight:800;color:white;margin-bottom:4px;letter-spacing:0.02em;">
                GolfAI Command Centre
            </div>
            <div style="color:#d8eee4;font-size:13px;margin-bottom:1px;">
                Session: mlm2pro_shotexport_011826.csv
            </div>
            <div style="color:#d8eee4;font-size:13px;">
                Shots Analysed: 53
            </div>
        </div>
        <div style="
